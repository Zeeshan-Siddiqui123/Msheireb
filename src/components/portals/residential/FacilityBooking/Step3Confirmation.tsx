...
const ApplePayLogo = () => (
  <svg width="38" height="16" viewBox="0 0 38 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <text x="0" y="13" fontSize="13" fontWeight="600" fill="white" fontFamily="SF Pro Display, sans-serif">Pay</text>
  </svg>
);
...
const Step3Confirmation = ({ onConfirm, onCancel }: Step3Props) => {
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <BaseCard>
        <CardHeader>
          <p className="text-small-regular font-semibold text-white mb-1">Silver Package - Cleaning Service</p>
        </CardHeader>
        <CardBody>
          <p className="text-small-regular text-white/70">
      ...
      <BaseCard>
        <CardHeader>
          <p className="text-small-regular text-white/70 mb-1">Service Delivery</p>
        </CardHeader>
      ...
      <BaseCard>
        <CardHeader>
          <p className="text-small-regular text-white/70 mb-1">Details</p>
        </CardHeader>
        <CardBody>
      ...
      <BaseCard>
        <CardHeader>
          <p className="text-small-regular text-white/70 mb-1">My information</p>
        </CardHeader>
        <CardBody>
      ...
      {/* Payment Options */}
      <BaseCard>
        <CardBody className="space-y-0 divide-y divide-white/10">
          {paymentOptions.map((option) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-inter-xs text-white/60">{option.description}</p>
              </div>

              {/* Add button or radio */}
              {option.hasAdd ? (
                <span className="text-small-regular text-white border border-white/40 rounded-full px-3 py-0.5 shrink-0">
                  Add
                </span>
              ) : (
                <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${
                  selectedPayment === option.key ? "border-white" : "border-white/40"
                }`}>
